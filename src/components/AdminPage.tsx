...
export function AdminPage({ onNavigate, currentUser }: AdminPageProps) {
  ...
  return (
    <>
      <SEOHead
        title="Admin Panel"
        description="PrivacyGuard administration panel. Manage users, roles, and permissions."
        path="/admin"
        noIndex={true}
      />
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <UserCheck className="w-8 h-8 text-blue-500" aria-hidden="true" />
                </div>
              </CardContent>
            </Card>
          </div>
                  ...
                  <table className="w-full" role="table">
                    <thead>
                      <tr className="border-b">
                        <th scope="col" className="text-left py-3 px-4 font-semibold text-sm">ID</th>
                        <th scope="col" className="text-left py-3 px-4 font-semibold text-sm">Username</th>
                        <th scope="col" className="text-left py-3 px-4 font-semibold text-sm">Email</th>
                        <th scope="col" className="text-left py-3 px-4 font-semibold text-sm">Role</th>
                        <th scope="col" className="text-right py-3 px-4 font-semibold text-sm">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {users.map((u) => {
                          ...
                          <tr key={u.id} className="border-b hover:bg-accent/50 transition-colors">
                            ...
                            <td className="py-3 px-4">
                              <div className="flex items-center justify-end gap-2">
                                {u.role === 'user' ? (
                                  <Button size="sm" variant="outline" onClick={() => handleChangeRole(u.id, 'admin')} aria-label={`Promote ${u.username} to admin`}>
                                    <UserCheck className="w-4 h-4 mr-1" aria-hidden="true" />
                                    Make Admin
                                  </Button>
                                ) : (
                                  <Button size="sm" variant="outline" onClick={() => handleChangeRole(u.id, 'user')} disabled={isSelf} aria-label={isSelf ? "Cannot demote yourself" : `Demote ${u.username} to user`}>
                                    <UserX className="w-4 h-4 mr-1" aria-hidden="true" />
                                    Make User
                                  </Button>
                                )}
                                <Button size="sm" variant="outline" onClick={() => handleDeleteUser(u.id, u.username)} disabled={isSelf} className="hover:bg-destructive/10 hover:text-destructive" aria-label={isSelf ? "Cannot delete yourself" : `Delete user ${u.username}`}>
